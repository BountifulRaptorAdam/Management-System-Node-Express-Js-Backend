...
// Change the two api endpoints to be one as having two endpoints is inefficient
    ...
    connection.query( query, [id], (error, results) => {
        if (!error) {
             if ( results.affectedRows == 0 ) {
                return res.status(404).json({message: 'Bill does not exist. Please Enter again'})
             } 

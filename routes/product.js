...
router.post('/add', auth.authenticateToken, checkRole.checkRole, (req, res) => {
    ...
    connection.query(query, [product.name, product.categoryId, product.description, product.price], (error, results) => {
        if (!error) {
            return res.status(200).json({ message: "Product Successfuly Added" });
        } else return res.status(500).json(error);
    })
})

router.get('/get', auth.authenticateToken, (req, res, next) => {
    var query = 'select p.id, p.name, p.description, p.price, p.status, c.id as categoryId, c.name as catagoryName from product as p INNER JOIN category as c where p.categoryId = c.id';
